  ...
  return (
    <div className="w-full h-screen fixed top-0 left-0 bottom-0 right-0  z-40 bg-black flex justify-center items-center">
      <div className=" absolute z-50 top-6 right-6">
        <h2
          className="text-gray-100 bg-gray-700 p-2 rounded-full cursor-pointer hover:text-yellow-400 hover:bg-gray-700 hover:shadow-lg hover:shadow-yellow-400 "
          onClick={() => closeModal()}
        >
          X
        </h2>
        ...
        {newArrayImg.length > 0
          ? newArrayImg.map((image, index) => (
              <img
                ...
                src={image}
                alt={image}
                className="w-5/12 h-4/5 rounded-md"
              />
            ))
